...
const FactionHeroCard = React.memo(function FactionHeroCard({ hero, owned, color, onHeroPress, cardW, cardH, scale }) {
  ...
  return (
    <TouchableOpacity
      style={[S.heroCard, { width: cardW }]}
      onPress={() => onHeroPress(hero)}
      activeOpacity={0.8}
    >
      {/* Card portrait */}
      <View style={[S.heroCardFrame, { width: cardW, height: cardH, borderColor: owned ? color + '99' : C.BORDER }]}>
        ...
        {!owned && (
          <View style={S.lockOverlay}>
            <View style={S.lockIcon}>
              <Ionicons name="lock-closed" size={rs(rsLocal(14))} color={C.TEXT_MUTED} />
            </View>
          </View>
        )}
        ...
        <View style={[S.rankBadge, { backgroundColor: r.bg, shadowColor: r.glow }]}>
          <Text style={[S.rankText, { color: r.text, fontSize: rf(rsLocal(8)) }]}>{hero.rank}</Text>
        </View>
      ...
      <Text style={[S.heroCardClass, { color: owned ? C.TEXT_SOFT : C.TEXT_DISABLED, fontSize: rf(rsLocal(8)) }]} numberOfLines={1}>
        {hero.class}
      </Text>
    </TouchableOpacity>
  );
...
function FactionScreen({ faction, heroes, ownedHeroes, color, onClose, onHeroPress }) {
  ...
  // Left column: 33% of W, hard-capped at 340 so it never dominates on large screens
  const LEFT_COL_W = Math.min(Math.floor(W * 0.33), 340);
  const sovereignPortraitW = Math.min(rs(rsLocal(130)), Math.floor(LEFT_COL_W * 0.50));
  const sovereignPortraitH = Math.floor(sovereignPortraitW * 1.26);

  // Right column: start with an estimate so first render isn't zero-width,
  // then update from the actual measured layout.
  const [rightColW, setRightColW] = useState(
    () => W - Math.min(Math.floor(W * 0.33), 340) - 1 - OUTER_PAD * 2 - COL_GAP * 2,
  );
  const onRightLayout = useCallback(
    e => setRightColW(e.nativeEvent.layout.width),
    [],
  );
    ...
    <View style={S.fsRoot}>
      ...
      >
        ...
        <View style={S.fsHeaderInfo}>
          <Text style={[S.fsHeaderName, { color, fontSize: rf(rsLocal(20)), letterSpacing: 4 }]}>{faction}</Text>
          <Text style={[S.fsHeaderClimate, { fontSize: rf(rsLocal(10)) }]}>📍 {meta.climate}   ·   {meta.element}</Text>
        </View>

        {/* Owned count */}
        <View style={[S.fsOwnedBadge, { borderColor: color + '55', backgroundColor: color + '18' }]}>
          <Ionicons name="people" size={rs(rsLocal(12))} color={color} />
          <Text style={[S.fsOwnedText, { color, fontSize: rf(rsLocal(11)) }]}>  {ownedCount} / {heroes.length}  OWNED</Text>
        </View>
        ...
        >
          <View style={[S.fsCloseBtnInner, { borderColor: C.BORDER, width: rs(rsLocal(52)), height: rs(rsLocal(52)), borderRadius: rs(rsLocal(25)) }]}>
            <Ionicons name="close" size={rs(rsLocal(36))} color={C.TEXT_SOFT} />
          </View>
        </TouchableOpacity>
      </LinearGradient>
      ...
      <View style={[S.fsBody, {
        ...
        >
          ...
          {/* ── Sovereign ── */}
          <Text style={[S.fsSectionLabel, { fontSize: rf(rsLocal(9)), letterSpacing: 2.5 }]}>SOVEREIGN</Text>
          <View style={S.rulerCard}>
            {rulerHero && (
              <View style={[S.rulerPortraitFrame, {
                borderColor: statusColor + 'BB',
                width: sovereignPortraitW,
                height: sovereignPortraitH,
            ...
            <View style={S.rulerInfo}>
              <Text style={[S.rulerName, { color, fontSize: rf(rsLocal(13)) }]}>{meta.ruler.name}</Text>
              <Text style={[S.rulerTitle, { fontSize: rf(rsLocal(9)), lineHeight: rf(rsLocal(13)) }]}>{meta.ruler.title}</Text>
              <View style={[S.rulerStatusBadge, { borderColor: statusColor + '60', backgroundColor: statusColor + '20' }]}>
                <Text style={[S.rulerStatusText, { color: statusColor, fontSize: rf(rsLocal(8)) }]}>{meta.ruler.status}</Text>
              </View>
            </View>
          </View>
          {/* Ruler role lore */}
          <Text style={[S.rulerLore, { fontSize: rf(rsLocal(10)), lineHeight: rf(rsLocal(15)) }]} numberOfLines={3}>{meta.ruler.roleLore}</Text>

          {/* Divider */}
          <View style={[S.fsDivider, { backgroundColor: color + '35' }]} />

          {/* ── Region traits ── */}
          <Text style={[S.fsSectionLabel, { fontSize: rf(rsLocal(9)), letterSpacing: 2.5 }]}>REGION TRAITS</Text>
          <View style={S.specialtyRow}>
            {meta.specialties.map((tag) => (
              <View key={tag} style={[S.specialtyTag, { borderColor: color + '60', backgroundColor: color + '18' }]}>
                <Text style={[S.specialtyText, { color, fontSize: rf(rsLocal(9)) }]}>{tag}</Text>
              </View>
            ))}
          </View>
          ...
          {/* ── Region lore ── */}
          <Text style={[S.fsSectionLabel, { fontSize: rf(rsLocal(9)), letterSpacing: 2.5 }]}>REGION LORE</Text>
          <Text style={[S.loreText, { fontSize: rf(rsLocal(11)), lineHeight: rf(rsLocal(17)) }]}>{meta.lore}</Text>
        </ScrollView>
        ...
        <View style={S.fsRight} onLayout={onRightLayout}>
          <View style={S.championsHeader}>
            ...
            <Text style={[S.championsSubtitle, { fontSize: rf(rsLocal(9)) }]} numberOfLines={1}>
              {heroes.length} heroes in {faction}
            </Text>
          </View>
...
export default function WorldMapScreen({ navigation }) {
  const { width: W, height: H } = useWindowDimensions();
  const factionZones   = computeZones(W, H);
  const factionMarkers = computeMarkers(W, H);
  ...
  return (
    <View style={S.root}>
      ...
      {FACTION_KEYS.map((key) => (
        <TouchableOpacity
          key={key}
          style={[S.touchZone, factionZones[key]]}
      ...
      {FACTION_KEYS.map((key) => {
        ...
        return (
          <Animated.View
            key={key}
            style={[S.markerWrap, factionMarkers[key], { transform: [{ scale: markerScale[key] }] }]}
            pointerEvents="none"
          >
            <Animated.View
      ...
      >
        <LinearGradient colors={C.GRAD_HEADER} style={S.backBtnGrad}>
          <Ionicons name="arrow-back" size={rs(20)} color={C.TEXT} />
...
const S = StyleSheet.create({
  ...
  fsHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    paddingHorizontal: rs(OUTER_PAD),
  ...
  // width set via inline style (live from useWindowDimensions)
  heroCard: {
    alignItems: 'center',
  },
  heroCardFrame: {
